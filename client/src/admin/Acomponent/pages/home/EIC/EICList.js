...
class EICList extends Component{
   constructor(props){
       super(props);
       this.state ={
        users: {},
...
getAllEICLists() {
  AdminAPI.getAllEICList()
        ...
        for(let i=0; i<res.data.data.length; i++){
            var stripedHtml = res.data.data[i].content.replace(/<[^>]+>/g, '');

             RowArray.push({title:res.data.data[i].title,content:stripedHtml,image:<img style={{width: 100}} src={process.env.PUBLIC_URL + '/upload-file/'+res.data.data[i].image}/>,action:[<Button onClick={this.EditEIC.bind(this, res.data.data[i])} variant="success" size="sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></Button>,<Button onClick={this.delEIC.bind(this, res.data.data[i]._id, i)}  variant="danger" size="sm"><i class="fa fa-trash-o" aria-hidden="true"></i></Button>]}) 
          }
...
AdminAPI.AdminEditEICInfo(EICInfoVo)
.then((result) => {
    this.getAllEICLists();
...
delEIC(id) {
    AdminAPI.AdmindeleteEICById(id)
      .then(res => {
         this.getAllEICLists();
    ...
    return(
        <div className="dashboard-section">   
            ...
            <div class="dashboard-content">
              <div className="heading"><h4>EIC lists</h4></div>
