...
class Serviceheader extends Component{
    constructor(props){
        ...
        this.AddServiceheaderFuc = this.AddServiceheaderFuc.bind(this);
    ...
    API.getServiceheader()
          .then(res => {
            ...
            this.setState({ 
                ...
                footercontent: res.data.data[0].footercontent,
             });
          }).catch(err => {
...
AddServiceheaderFuc(e){
        var serviceInfoVo = new FormData()
        serviceInfoVo.append('title', this.refs.title.value)
        serviceInfoVo.append('content', this.refs.content.value)
        serviceInfoVo.append('footercontent', this.refs.footercontent.value)
        serviceInfoVo.append('image', this.state.EICImage)
        serviceInfoVo.append('oldimage', this.refs.oldimage.value)
        serviceInfoVo.append('id', this.refs.id.value)
        serviceInfoVo.append('datetime', new Date())

        API.AddServiceheader(serviceInfoVo).then((result) => {
            this.getServiceheaders()
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="panel panel-default">
                                                <div class="panel-heading clearfix">
                                                    ...
                                                    <h5 class="panel-title">SERVICE PAGE CONTENT</h5>
                                                </div>
                                                ...
                                                <form role="form">
                                                    ...
                                                    <div class="form-group">
                                                        <label>Service Footer Content</label>
                                                        <textarea className={this.state.errors["footercontent"] ? this.state.errors["footercontent"] : 'form-control'} onChange={this.handleChange.bind(this, "footercontent")} ref="footercontent" id="footercontent" name="footercontent" rows="4" cols="50"  value={this.state.fields["footercontent"] ? this.state.fields["footercontent"] : this.state.footercontent } />
                                                    </div>

                                                    <button type="button" onClick={this.AddServiceheaderFuc} class="btn btn-success" title="">Submit</button>
                                                </form>
                                                </div>
                                            </div>
                                            </div>
